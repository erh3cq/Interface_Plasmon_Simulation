...
system = System(keV=100,
                q_perpendicular_max=6E10,
                E_min=5,
                E_max=20)
system.configure_traits()

#MR_Spectra = double_differential_cross_section_normalIncidence(system.microscope, system.dimensions, [vac, Al], 100*10E-9)

microscope = Microscope(keV=100)
scope.configure_traits()
#print('theta_max: %.4f'%(dimensions.q_perpendicular.max()/scope.k0))
#print('(111) @',2*np.pi/2.338,'[1/A] \n')
dimensions = Spectrum_dimensions(q_perpendicular_max=6E10, E_min=5, E_max=20)
#dimensions.configure_traits()
MR_Spectra = double_differential_cross_section_normalIncidence(microscope, dimensions, [vac, Al], 100*10E-9)
